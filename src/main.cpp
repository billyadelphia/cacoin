...
#if defined(NDEBUG)
# error "Cacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa40bf8048601d99584c068d263aa782debf01c0ed934655e9a9210b538b9d29a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cacoin Signed Message:\n";
...
{
    // Cacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 0 * COIN;
        if (nHeight <= 500) { nSubsidy = 5000 * COIN; }
        else if (nHeight <= 1500) { nSubsidy = 3500 * COIN; }
        else if (nHeight <= 3000) { nSubsidy = 2000 * COIN; }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5cc324b41c0ca72dcb9fbe09c0615364c8c253b96f56fb1f4bcaf066f9156bdb");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Crypto Academy Coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 0 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0421299c92b8d3bebcf2a64b6dcc1bfa951ad37fd1a2e2dcb902d8db25efea1d5e9da56417e41df7327c177d36111737c236698023114d6e41b5a7e1a1410f44b7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1562293293;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1842212;

        if (fTestNet)
        {
            block.nTime    = 1559812149;
            block.nNonce   = 49827;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xc5cc88bdcdda891511481dec0250c67d19a07b025bcda368b06289c720e36b2d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc5, 0xbb, 0xde };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CacoinMiner(CWallet *pwallet)
{
    printf("CacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cacoin-miner");
    ...
    try { loop {
        ...
        printf("Running CacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CacoinMiner, pwallet));
}
